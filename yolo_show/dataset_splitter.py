...
class DatasetSelector(QWidget):
    ...
    def confirm_action(self):
        ...
        if train_path and test_path and val_path and train_count > 0 and test_count > 0 and val_count > 0:
            self.selected_images["train"] = self.select_images(train_path, train_count)
            self.selected_images["test"] = self.select_images(test_path, test_count)
            self.selected_images["val"] = self.select_images(val_path, val_count)
    ...
    def select_images(self, path, count):
    ...
    def write_selected_images(self):
        ...
        for category in ["train", "test", "val"]:
            ...
            dst_labels_path = os.path.join(base_dir, "labels")
            ...
            label_file_path = os.path.join(dst_labels_path, f"{category}.txt")
            with open(label_file_path, "w") as label_file:
                for image in self.selected_images[category]:
                    src_file = os.path.join(src_path, image)
                    dst_file = os.path.join(dst_images_path, image)
                    shutil.copy(src_file, dst_file)
                    label_file.write(dst_file + "\n")
