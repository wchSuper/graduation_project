...
class CameraDetectThread(QThread):
    ...
    on_camera_signal = pyqtSignal(list)
    on_camera_transfer = pyqtSignal(list)

    def __init__(self, info):
        super(CameraDetectThread, self).__init__()
        self.info_list = info
        self.iou_value = info[0]
        self.conf_value = info[1]
        self.weight = info[2]

        self.on_camera_signal.connect(self.send_to_mainThread)

    def run(self):
        detect(self.iou_value, self.conf_value, self.weight, self.on_camera_signal)

    def send_to_mainThread(self, pix_list):
        self.on_camera_transfer.emit(pix_list)


def detect(iou_value, conf_value, weight, camera_signal=""):
    opt = getOpt()
    source, weights, view_img, save_txt, imgsz, trace = opt.source, opt.weights, opt.view_img, opt.save_txt, opt.img_size, not opt.no_trace
    save_img = True
    webcam = 1
    opt.conf_thres = conf_value
    opt.iou_thres = iou_value
    opt.weights = weight
    ...
    for path, img, im0s, vid_cap in dataset:
        ...
        # Apply Classifier  classify == False
        # 无
